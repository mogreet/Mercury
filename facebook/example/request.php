...
if (!isset($action, $client_id, $token)) {
    printError("Action, client id and token must be set.");
    exit;
}

/* Instantiates the Mercury object and calls the appropriate method */
$mercury = new Mercury($client_id, $token);
try {
    // removes parameters to avoid double
    ...
    switch (strtoupper($action)) {
        case 'PING':
            $response = $mercury->ping();
            break;
        case 'SEND':
            $response = $mercury->send($_POST);
            break;
        case 'LOOKUP':
            $response = $mercury->lookup($_POST);
            break;
        case 'GETOPT':
            $response = $mercury->getopt($_POST);
            break;
        case 'SETOPT':
            $response = $mercury->setopt($_POST);
            break;
        case 'UNCACHE':
            $response = $mercury->uncache($_POST);
            break;
        case 'INFO':
            $response = $mercury->info($_POST);
            break;
        case 'TRANSACTIONS':
            $response = $mercury->transactions($_POST);
            break;
